...
export const AdBanner: React.FC<AdBannerProps> = ({ slotType = "leaderboard", className = "" }) => {
  const getSlotHeightClass = () => {
    switch (slotType) {
      case "leaderboard": return "min-h-[110px]";
      case "sidebar": return "min-h-[270px]";
      case "in-article": return "min-h-[140px]";
      case "mobile-banner": return "min-h-[120px]";
      default: return "min-h-[110px]";
    }
  };

  return (
    <div className={`my-4 overflow-hidden rounded-xl bg-neutral-100 border border-neutral-200/80 p-2 text-center ${getSlotHeightClass()} ${className}`}>
      ...
      <div className={`flex items-center justify-center text-xs text-neutral-400 font-medium bg-neutral-50/80 rounded border border-dashed border-neutral-300 ${getSlotHeightClass()}`}>
